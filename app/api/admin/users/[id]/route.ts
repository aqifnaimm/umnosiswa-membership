  ...
  try {
    ...
    if (target.auth_user_id === me.userId) {
      return NextResponse.json(
        {
          error:
            "Anda tidak boleh mengubah role/status akaun sendiri di sini."
        },
        { status: 400 }
      );
    }
    ...
    if (
      ...
      if ((count || 0) <= 1) {
        return NextResponse.json(
          {
            error:
              "Sistem mesti mempunyai sekurang-kurangnya seorang Super Admin aktif."
          },
          { status: 400 }
        );
      }
