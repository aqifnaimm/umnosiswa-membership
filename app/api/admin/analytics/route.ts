...
async function current(req: Request) {
  ...
  return profile ? { email:profile.email, role:profile.role } : null;
}
...
export async function GET(req: Request) {
  try {
    ...
    return NextResponse.json({
      members:data || [],
      admin:{email:me.email,role:me.role}
    });
  } catch (e:any) {
    return NextResponse.json({error:e.message || "Gagal mendapatkan analytics."},{status:500});
  }
}
